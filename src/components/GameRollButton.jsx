export default function GameRollButton({ onClick, disabled, isDesktop, isBotShooter, isBotActive, label, fullWidth }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      padding: isDesktop ? "14px 40px" : fullWidth ? "14px 20px" : "12px 32px",
      width: fullWidth ? "100%" : undefined,
      borderRadius: 10, fontSize: isDesktop ? 16 : 15, fontWeight: 700,
      ...
      transition: "all .3s", whiteSpace: fullWidth ? "normal" : "nowrap",
    }}>{label}</button>
  );
}
